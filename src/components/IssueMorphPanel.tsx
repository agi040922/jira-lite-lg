...
import { summarizeIssue, suggestSolution, askAIAboutIssue } from "@/app/actions/ai"
...
interface ContextShape {
  ...
  handleQuickAction: (action: 'summary' | 'suggestion') => Promise<void>
  resetResponse: () => void
}
...
export function IssueMorphPanel({ issueTitle, issueDescription }: IssueMorphPanelProps) {
  ...
  const handleQuickAction = React.useCallback(async (action: 'summary' | 'suggestion') => {
    setIsLoading(true)
    setResponse(null)
    setShowForm(true)
    ...
    try {
      const result = action === 'summary'
        ? await summarizeIssue(issueDescription || '')
        : await suggestSolution(issueTitle, issueDescription || '')
  ...
  const ctx = React.useMemo(
    () => ({ showForm, successFlag, triggerOpen, triggerClose, isLoading, response, handleSubmit, handleQuickAction, resetResponse }),
    [showForm, successFlag, triggerOpen, triggerClose, isLoading, response, handleSubmit, handleQuickAction, resetResponse]
  )
      ...
      {!showForm && !response && (
        <div className="flex gap-2 justify-center mt-3">
          <button
            ...
            className="bg-white text-brand-600 border border-brand-200 px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-brand-100 transition-colors shadow-sm"
            disabled={isLoading}
          >
            📝 내용 요약하기
          </button>
          <button
            ...
            className="bg-brand-500 text-white px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-brand-600 transition-colors shadow-sm"
            disabled={isLoading}
          >
            💡 해결 제안받기
          </button>
...
export default IssueMorphPanel
